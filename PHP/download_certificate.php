...
if(isset($_POST["email"])){
  ...
  if($toemail != ""){ 
   if(!$conn){
        ...
        if($count >0){
          echo "yes it is checking";
          header("Location:https://csgowtham.tech/New%20Project/PHP/otp.php");
          $stmt = $conn->prepare("insert into otp_verification(Email, otp) values(?,?)");
          $stmt->bind_param("si",$toemail,$otp);
          $stmt->execute();
          $stmt->close();
          mysqli_query($conn, "insert otp_verification set otp = '$otp' where Email = '$toemail'");
          if(mail($toemail,$subject,$Message, $headers)){
            $show .= "sent succefully";
          }else{
            $error .="Not Sent";
          }
...
<head>
    ...
    <link rel="stylesheet" href="../CSS/download_certificate.css">
        ...
        <center>
          ...
          <form method = "post" class="email-form">
             <div class="form-group">
              <label for="exampleInputEmail1"><strong>Enter Registered Email address</strong></label>
              <br>
              <br>
              <div class="col-xs-4">
            ...
            <br>
              <button type="submit" class="btn btn-primary email-submit">Send OTP</button>
          </form>
      </div>
        <div class="otp_container">
          <form method = "post" class="otp-form">
            <div class="form-group">
              ...
              <button type="submit" class="btn btn-primary submit-otp">Submit OTP</button>
          </form>
        </center>
        </div>
        <script>
